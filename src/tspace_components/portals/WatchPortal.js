...
const createRippleMaterial = () => {
  ...
  const imageTexture = textureLoader.load(process.env.PUBLIC_URL + '/assets/watchIcon.png');
...
const planeTexture = new THREE.TextureLoader().load(process.env.PUBLIC_URL + '/assets//UI/Theatre4.png');
...
const usePortalModel = (position, rotation, size, scene, rippleMaterial) => {
  ...
  const [envMap, setEnvMap] = useState();
    ...
    useEffect(() => {
    new RGBELoader().load(`${process.env.PUBLIC_URL}/assets/envTest.hdr`, (texture) => {
  ...
  useEffect(() => {
    ...
    const planeMaterialConfig = {
        transmission: 0, // Adjust the transmission value for more/less transparency
        roughness: 0.5,    // Adjust the roughness for more/less blurriness
        clearcoat: 1.0, 
        envMap: envMap   // Adjust clearcoat if needed
    };
    const planeMaterialOld = new THREE.MeshBasicMaterial({ map: planeTexture, side: THREE.DoubleSide, transparent: true });
      
    const planeMaterial = <MeshTransmissionMaterial {...planeMaterialConfig} side={THREE.DoubleSide} transparent={true} />;
    const planeGeometry = new THREE.PlaneGeometry(8, 4.5);  // Adjust the size as needed
    const planeMesh = new THREE.Mesh(planeGeometry, planeMaterialOld);

    // Position the plane above the portal
    planeMesh.position.set(...position);
    planeMesh.position.y += 7;  // Slightly above the portal to avoid z-fighting
    planeMesh.rotation.y = Math.PI;  // Rotate to face upwards
...
const WatchPortal = ({
  ...
  useFrame((state) => {
    ...
    const portalPosition = new THREE.Vector3().fromArray(position);;
